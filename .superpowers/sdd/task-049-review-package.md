...
6e1ca63 test: strengthen prediction failure coverage
fa9341b test: add prediction failure red tests

## 统计

 .superpowers/sdd/task-049-report.md       |  32 ++++
 tests/failure/test_prediction_failures.py | 294 ++++++++++++++++++++++++++++++
 2 files changed, 326 insertions(+)

## 差异

```diff
diff --git a/.superpowers/sdd/task-049-report.md b/.superpowers/sdd/task-049-report.md
new file mode 100644
index 0000000..3fb91b2
--- /dev/null
+++ b/.superpowers/sdd/task-049-report.md
@@ -0,0 +1,32 @@
+# T049 预测失败场景红灯测试报告
+
+## 交付内容
...
+- 覆盖到期结果只能以独立追加记录关联 `prediction_snapshot_id`；追加前后原快照的概率、规则/模型/数据版本与生成时点逐项不变，重复追加拒绝。
+- 将到期价格、公司行动、交易日历、标签规则的可得时点晚于验证边界，以及日历、规则版本不匹配，拆分为独立参数化拒绝用例，防止回写历史预测。
+
+## 红灯验证
+
+执行命令：
...
+## 后续关注项
+
+- T050/T052 需实现本测试声明的预测输入、不可变快照和独立到期回填边界后，再将本测试转为通过；到期结果必须以关联记录保存而不能更新预测快照。
+- 对无有效到期价格的场景，领域结果必须同时保留 `PENDING_VALIDATION` 与可审计的明确原因，不能以方向标签替代。
diff --git a/tests/failure/test_prediction_failures.py b/tests/failure/test_prediction_failures.py
new file mode 100644
index 0000000..dd4d904
--- /dev/null
+++ b/tests/failure/test_prediction_failures.py
@@ -0,0 +1,294 @@
